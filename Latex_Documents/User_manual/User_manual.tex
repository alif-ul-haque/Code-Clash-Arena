...
Quick setup guide to access CodeClash Arena and create your account.

\setlength{\parindent}{0pt}

\subsection{Accessing the Platform}

\subsubsection{To Access the Platform:}

\begin{itemize}[leftmargin=*]
\item Open a supported web browser (Chrome 84+, Firefox 103+, Edge 84+, Safari 15+)
\item Navigate to: \url{https://code-clash-arena-six.vercel.app/}
...
\textbf{Step 4: Email Verification}
\begin{itemize}[leftmargin=*]
...
\textbf{Step 5: Login to Complete Setup}
\begin{itemize}[leftmargin=*]
...
\textbf{Step 3: Submit and Authenticate}
\begin{itemize}[leftmargin=*]
...
\textbf{Step 4: Access Dashboard}
\begin{itemize}[leftmargin=*]
\item After 1.5 seconds, you are redirected to the Main Dashboard
\item Your Codeforces handle is automatically loaded from your account
\item All platform features are accessible: PvP Battles, Practice Gym, Clan Wars, Leaderboards
\end{itemize}
...
\item \textbf{Network error} - Check internet connection and refresh page
\item \textbf{Forgot password} - Use password reset option on login page
\end{itemize}
...
\subsubsection{To Logout:}

\begin{itemize}[leftmargin=*]
\item Click your username or profile icon in the top-right corner
\item Select \textbf{"Logout"} or \textbf{"Sign Out"} from the dropdown menu
\item Confirm logout if prompted
\item You are redirected to the homepage with all session data cleared
\end{itemize}
...
\textbf{Security:} Always logout on shared computers to prevent unauthorized access.
...
\subsubsection{Continue Your Journey:}

\begin{itemize}[leftmargin=*]
\item Verify your Codeforces handle in your profile settings
\item Explore the \textbf{Practice Gym} to familiarize yourself with the code editor
\item Join or create a \textbf{Clan} for team-based competitions
\item Join your first \textbf{1v1 Battle} to start competing
\end{itemize}
